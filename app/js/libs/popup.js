...
export default class Popup {
    ...
    // Встановлення хеша
    _setHash() {
        if (!window.location.href.includes(this.hash)) {
            history.pushState({}, false, window.location.href + this.hash);
        }
    }
    _removeHash() {
        history.pushState({ hash: this.hash }, true, window.location.href.split('#')[0])
    }
    _focusCatch(e) {
